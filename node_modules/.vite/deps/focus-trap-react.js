import { a as __toCommonJS, i as __exportAll, n as __commonJSMin, r as __esmMin, t as require_react } from "./react-BoDunMG2.js";
...
var init_index_esm = __esmMin((() => {
	...
	focusableCandidateSelector = /* @__PURE__ */ candidateSelectors.concat("iframe:not([inert]):not([inert] *)").join(",");
	isFocusable = function isFocusable(node, options) {
		options = options || {};
		if (!node) throw new Error("No node provided");
...
var focus_trap_esm_exports = /* @__PURE__ */ __exportAll({ createFocusTrap: () => createFocusTrap });
...
var init_focus_trap_esm = __esmMin((() => {
	init_index_esm();
	activeFocusTraps = {
	...
	isSelectableInput = function isSelectableInput(node) {
		return node.tagName && node.tagName.toLowerCase() === "input" && typeof node.select === "function";
	};
	isEscapeEvent = function isEscapeEvent(e) {
...
//#endregion
//#region node_modules/focus-trap-react/dist/focus-trap-react.js
var require_focus_trap_react = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	function _typeof(o) {
	...
	var createFocusTrap = (init_focus_trap_esm(), __toCommonJS(focus_trap_esm_exports)).createFocusTrap;
	var isFocusable = (init_index_esm(), __toCommonJS(index_esm_exports)).isFocusable;
	/**
		...
		function FocusTrap(props) {
			...
			*/
			...
			for (var optionName in focusTrapOptions) {
				if (!Object.prototype.hasOwnProperty.call(focusTrapOptions, optionName)) continue;
		...
		return _createClass(FocusTrap, [
			...
			},
			{
				...
				value: function updatePreviousElement() {
					...
					if (currentDocument) this.previouslyFocusedElement = currentDocument.activeElement;
				}
			},
			{
				key: "deactivateTrap",
			...
			},
			{
				...
				value: function handleClickOutsideDeactivates(event) {
					...
					if (allowDeactivation) this.outsideClick = {
						target: event.target,
						allowDeactivation
					};
			...
			},
			{
				...
				value: function handleDeactivate() {
					if (this.originalOptions.onDeactivate) this.originalOptions.onDeactivate.call(null);
					this.deactivateTrap();
				}
			},
			{
				...
				value: function handlePostDeactivate() {
					...
					var finishDeactivation = function finishDeactivation() {
						...
						if (canReturnFocus) returnFocusNode.focus({ preventScroll });
						if (_this2.originalOptions.onPostDeactivate) _this2.originalOptions.onPostDeactivate.call(null);
						_this2.outsideClick = null;
					};
					if (this.originalOptions.checkCanReturnFocus) this.originalOptions.checkCanReturnFocus.call(null, this.getReturnFocusNode()).then(finishDeactivation, finishDeactivation);
					else finishDeactivation();
				}
			},
			{
				key: "setupFocusTrap",
				value: function setupFocusTrap() {
					...
					} else if (this.focusTrapElements.some(Boolean)) {
			...
			},
			{
				...
				value: function componentDidMount() {
					if (this.props.active) this.setupFocusTrap();
				}
			},
			{
				...
				value: function componentDidUpdate(prevProps) {
					if (this.focusTrap) {
						if (prevProps.containerElements !== this.props.containerElements) this.focusTrap.updateContainerElements(this.props.containerElements);
						...
						if (hasPaused) this.focusTrap.pause();
						if (hasUnpaused) this.focusTrap.unpause();
					} else {
						if (prevProps.containerElements !== this.props.containerElements) this.focusTrapElements = this.props.containerElements;
			...
			},
			{
				...
				value: function render() {
					...
					if (child) {
						if (child.type && child.type === React.Fragment) throw new Error("A focus-trap cannot use a Fragment as its child container. Try replacing it with a <div> element.");
						return React.cloneElement(child, { ref: function callbackRef(element) {
							var containerElements = _this3.props.containerElements;
							if (child) {
								if (reactVerMajor >= 19) {
									if (typeof child.props.ref === "function") child.props.ref(element);
									else if (child.props.ref) child.props.ref.current = element;
								} else if (typeof child.ref === "function") child.ref(element);
								else if (child.ref) child.ref.current = element;
							}
